class Viewer{    constructor(list,id,opts){		// list: a table of images pathname to be viewed		// id: instance id for cookie management		// opts: options to be passed as a key/value dictionnary
        var that = this;
	    this._id = id;
        this._list = list;
        ...
        this._isMovingImg = false;				// init options with defaults parameters and then overwrite with values in opts argument        Viewer.deepClone(this._opts,Viewer.defaultOpts);
        Viewer.deepClone(this._opts,opts);
        
		// get cookie to resume the comic where it has been closed
        ...
        this._progressBar.className = 'cv-progress-bar';				// create navigation bar buttons        this._navbarbutton('','cv-navigation-bar-button-previous',function (){
            that.backward();
        });
        this._navbarbutton('','cv-navigation-bar-button-next',function (){
            that.forward();
        });
        this._navbarbutton('','cv-navigation-bar-button-rewind',function (){
            that.rewind();
        });
        this._navbarbutton('','cv-navigation-bar-button-fullscreen',function (){
            that.toggleFullscreen();
        });
        this._navbarbutton('','cv-navigation-bar-button-magnify',function (){
            that.toggleAdjustToViewport();
        });
        this._downButton = this._navbarbutton('','cv-navigation-bar-button-download',function (){
        ...
        this._contextmenubutton('next','->',function(){
            ...
            that._onresize()
        });				// update cookies before closing app		window.addEventListener('beforeunload', function(e) {
